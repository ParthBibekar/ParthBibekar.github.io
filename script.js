...
function updateRatings(userData) {
  document.getElementById("lichess-rapid").innerHTML =
    userData["perfs"]["rapid"]["rating"];
  document.getElementById("lichess-blitz").innerHTML =
    userData["perfs"]["blitz"]["rating"];
  document.getElementById("lichess-bullet").innerHTML =
    userData["perfs"]["bullet"]["rating"];
  document.getElementById("lichess-puzzle").innerHTML =
    userData["perfs"]["puzzle"]["rating"];
  document.getElementById("lichess-960").innerHTML =
    userData["perfs"]["chess960"]["rating"];
  document.getElementById("lichess-puzzle-storm").innerHTML = userData["storm"][
    "runs"
  ]
    ? userData["storm"]["runs"]
    : 0; // Assuming 'runs' gives number of puzzle storms
}
...
async function plotRatingHistory(username) {
  ...
  data.forEach((category) => {
    if (categoriesToPlot.includes(category.name)) {
      ...
      let dates = category.points.map(
        (point) => new Date(point[0], point[1], point[2]),
      ); // Construct dates from points
